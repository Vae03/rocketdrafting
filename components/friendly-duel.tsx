        ...
        <div className="vs-columns">
          <div className="vs-column">
            ...
            {result ? <section className="match-result-panel">
              ...
              : currentStep === "ORG" ? <section className="pick-zone"><div className="pick-heading"><span>{orgStepInfo.accent}</span><div><p className="kicker">FRIENDLY DRAFT</p><h2>{orgStepInfo.title}</h2></div><div className="pick-count">PICK <b>1</b> / 6</div></div><div className="cards">{orgOffer.map((org, index) => <OrgCard key={org.id} org={org} index={index} showBonus onPick={() => pickOrg(org)} />)}</div></section>
